...
def _seed(session: Session) -> Store:
    store = Store(chain="lidl", name="Test Lidl", plz="10713")
    session.add(store)
    session.flush()
    session.add_all(
        [
...
def test_reset_wipes_then_rescrapes(monkeypatch):
    ...
    result = trigger_reset(session, plz="10713", token=None)
...
def test_reset_requires_token_when_configured(monkeypatch):
    ...
    with pytest.raises(HTTPException) as exc:
        trigger_reset(session, plz="10713", token=None)
    assert exc.value.status_code == 403
    assert len(session.scalars(select(Offer)).all()) == 2

    # correct token proceeds with the wipe
    result = trigger_reset(session, plz="10713", token="secret")
    assert result["deleted"] == 2
